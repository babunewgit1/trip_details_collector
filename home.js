(function () {
   function initSearchWidget(root) {
      ...
      function getAircraftType(btn) {
         // Walk up from the button to find the parent that holds
         // both the submit and the radio section for this tab.
         var parent = btn.closest(".tab_cnt_item")
            || btn.closest(".algolio_submit")
            || btn.closest(".hmtrip")
            || document;

         // The radio section (.tcradio) may be a sibling of the
         // .algolio_submit, so check the parent element too.
         var scope = parent;
